<?php 
	$top_background = "#2aa48c";
// 	$top_background = "#1e4169";
    $theme_main_color = "#233646";
    $theme_second_color = "#2aa48c";
    $header_text_color = "#00224f";
    $bg_color_2 = "#2aa48c";//light color
    $bg_color_1 = "#e6ecf2";//drak color
    $footer_bottom = "#7a030d";
    $light_main_color = "#dbaeba";
?>
<style type="text/css">
.main-header.style-two .header-top{
	background: <?=$top_background?> !important;
}
.main-menu .navigation>li>ul>li>a, .main-menu .navigation>li>.megamenu li>a:hover{
    color : white !important;
}
.pricing-block-one .pricing-table .table-content ul li:before{
    color: <?=$top_background?> !important;
}
.theme-btn-one:hover{
     background: <?=$theme_second_color?> !important;
     	box-shadow: 0 20px 30px <?=$theme_second_color?> !important;
}
.e-nav-cricle{
    background: <?=$theme_second_color?> !important;
}

.registration-section .content-box .title-box {
    	background: <?=$top_background?> !important;
}
.feature-block-two .inner-box .icon-box{
     color: <?=$top_background?> !important;
}
.profile-box .profile-info .list li a.current i, .profile-box .profile-info .list li a:hover i{
    color: <?=$top_background?> !important;
}
.theme-btn-one:before{
    background: <?=$theme_second_color?> !important;
}
.registration-section .content-box .title-box h3{
    color : white !important;
}
.forgot_line{
    color: <?=$top_background?> !important;
}
.registration-section .content-box .login-now p a{
    color: <?=$top_background?> !important;
}
.registration-section .content-box .title-box a:hover{
    color :<?=$theme_second_color?> !important;
}
.registration-section .content-box .title-box a{
    color :<?=$theme_second_color?> !important;
}
.custom-controls-stacked .description a {
    color: <?=$theme_second_color?> !important;
    border-bottom: 1px solid <?=$theme_second_color?> !important;
}

#totalcart{
    color: white !important;
}
.image_block_3 .image-box .pattern .pattern-3{
    background: <?=$top_background?> !important;
}


.main-menu .navigation > li > ul, .main-menu .navigation > li > .megamenu{
    background: <?=$top_background?> !important;
}

.header-top .info li a:hover{
    color : <?=$top_background?> !important;
}
.main-menu .navigation > li > a:before{
    background : <?=$theme_second_color?> !important;
}

.main-header.style-two .header-top .top-left li i{
	color: <?=$bg_color_1?> !important;
}

.main-header .btn-box .theme-btn-one{
	box-shadow : 0 10px 10px <?=$theme_second_color?> !important;
}
.theme-btn-one{
	background: <?=$theme_second_color?> !important;
	box-shadow: 0 20px 30px <?=$theme_second_color?> !important;
}
.main-menu .navigation > li.current > a, .main-menu .navigation > li:hover > a{
	color: <?=$theme_main_color?> !important;
}
.main-menu .navigation > li > a:before{
	color: <?=$theme_main_color?> !important;
}
.main-menu .navigation > li.current > a, .main-menu .navigation > li:hover > a{
	color: <?=$top_background?> !important;
}

.main-menu .navigation > li > a:before{
	color: <?=$theme_main_color?> !important;
}

.banner-section.style-five .content-box{
	background: <?=$theme_second_color?> !important;
}

.banner-section .content-box form .form-group button{
	background : <?=$top_background?> !important;
}
h1, h2, h3, h4, h5, h6{
	color: #2aa48c  !important;
}
.category-block-one .inner-box .link a{
	background: <?=$light_main_color?> !important;
}
.category-block-one .inner-box .link a{
	color: <?=$top_background?> !important;
}
.pricing-block-one .pricing-table .table-footer .link a{
	background: <?=$light_main_color?> !important;
}
.pricing-block-one .pricing-table .table-footer .link a{
	color: <?=$top_background?> !important;
}
.bg-color-2{
	background-color: <?=$bg_color_2?> !important;;
}
.bg-color-1{
	background-color: <?=$bg_color_1?> !important;;
}
.bg-color-3{
	background-color: <?=$bg_color_1?> !important;;
}
.counter-block-one .inner-box h4{
	color: white !important;
}
.sec-title p{
	color: <?=$theme_main_color?> !important;
}
.sec-title.light h2{
	color: white !important;
}
.content_block_2 .content-box .btn-box .download-btn.app-store{
	background: <?=$theme_main_color?> !important;
}
.content_block_2 .content-box .btn-box .download-btn.play-store{
	background: <?=$top_background?> !important;
}
.content_block_2 .content-box .btn-box .download-btn h3{
	color: white !important;
}
.content_block_4 .content-box h3{
	color: white !important;
}
.content_block_3 .content-box h3{
	color: white !important;
}
.content_block_4 .content-box .subscribe-form .form-group button{
	background: <?=$top_background?> !important;
}
.content_block_3 .content-box:before, .content_block_4 .content-box:before{
	background: <?=$top_background?> !important;
}
.content_block_3 .content-box .support-box h3 a{
	color: <?=$bg_color_1?> !important;
}
.content_block_3 .content-box .support-box .icon-box{
	color: <?=$bg_color_1?> !important;
}
.footer-top{
	background: <?=$top_background?> !important;
}
.footer-top .widget-title h3{
	color: white !important;
}
.footer-top .contact-widget .widget-content li i{
	color : <?=$bg_color_1?> !important;
}
.counter-block-one .inner-box .count-outer{
	color: <?=$bg_color_1?> !important;
}
.footer-bottom{
	background: <?=$top_background?> !important;
}
.banner-section.style-five .content-box h2{
	color : white !important;
}
.page-title-two .title-box h1{
	color : white !important;
}
.content_block_4 .content-box .subscribe-form .form-group button:hover{
	color: <?=$top_background?> !important;
}
.feature-block-one .inner-box .link a{
	background: <?=$light_main_color?> !important;
	color : <?=$top_background?> !important;
}
.sm-container i {
    	color : <?=$bg_color_1?> !important;
    	background-color:<?=$theme_second_color?> !important;
}
.clinic-block-one .inner-box .content-box .like-box a{
    	color : <?=$bg_color_1?> !important;
    	background-color:<?=$theme_second_color?> !important;
}
.sec-title p{
	color: #2aa48c !important;
}
.list-style-one li:before {
    border: 3px solid <?=$top_background?> !important;
}
.footer-top .links-widget .links li a:before{
	background: <?=$theme_main_color?> !important;
}
.clinic-details-content .tabs-box .tab-btn-box{
	background: <?=$theme_second_color?> !important;
}
.clinic-details-content .tabs-box .tab-btn-box .tab-btns li.active-btn{
	color: white !important;
}
.panel-title > a{
	background: #FFF !important;
	border:0.5px solid gray;
}
.star-rating label:hover, .star-rating label:hover ~ label{
	color: <?=$top_background?> !important;
}
.star-rating :checked ~ label{
	color: <?=$top_background?> !important;
}
.single-review-box .rating li{
	color: <?=$top_background?> !important;
}
.clinic-sidebar .form-widget .form-title{
	background: <?=$top_background?> !important;
}
.clinic-sidebar .form-widget .form-title h3{
	color: white !important;
}
.team-block-three .inner-box{
	background: <?=$theme_second_color?> !important;
    color: white;
}
.team-block-three .inner-box .lower-content .name-box li h3 a{
	color: white !important;
}
#my_member_list{
	padding: 25px;
}
.panel-default > .panel-heading + .panel-collapse > .panel-body{
	padding: 25px ;
}
.scroll-top {
    background: <?=$top_background?> !important;
}
.profile-box .profile-info .list li a.current, .profile-box .profile-info .list li a:hover{
	color: <?=$theme_main_color?> !important;
}
.theme-btn-two:hover{
	background: <?=$theme_main_color?> !important;
	border-color: <?=$theme_main_color?> !important;
}
.theme-btn-two{
	color: <?=$theme_main_color?> !important;
}
.dataTables_wrapper .dataTables_paginate .paginate_button.disabled, .dataTables_wrapper .dataTables_paginate .paginate_button.disabled:hover, .dataTables_wrapper .dataTables_paginate .paginate_button.disabled:active{
	background: <?=$theme_second_color?> !important;
}
.dataTables_wrapper .dataTables_paginate .paginate_button.current, .dataTables_wrapper .dataTables_paginate .paginate_button.current:hover{
	border: 1px solid <?=$top_background?> !important;
    background: <?=$top_background?> !important;
}
</style>
  