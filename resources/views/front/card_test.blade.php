@php
  ...
  /* Included parameters — normalize whatever the backend provides:
     an array of ['name' => ...] rows, an array of strings, or a
     comma-separated string of names (as built for profiles/tests). */
  $hdParams = [];
...
@endphp
<div class="premium-test-card">
  <!-- Discount Badge -->
  @if($hdDiscount > 0)
    <div class="premium-test-discount">
  ...
  <!-- Card Header -->
  <div class="premium-test-header">
    <div class="premium-test-icon-wrapper">
      <i data-lucide="test-tubes" class="premium-test-icon"></i>
    </div>

    <h4 class="premium-test-name">{{ $pl->profile_name ?? $pl->name ?? '' }}</h4>

    <div class="premium-test-params">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
  ...
  <!-- Card Body -->
  <div class="premium-test-body">
    @if(count($hdParams) > 0)
    <div class="premium-test-features">
      <div class="premium-test-features-title">Test Includes</div>
      <ul class="premium-features-list">
        ...
        @if(count($hdParams) > 4)
          <li class="premium-feature-item premium-feature-more">
            ...
            +{{ count($hdParams) - 4 }} more parameters
          </li>
        @endif
      </ul>
    </div>
  ...
  <!-- Price Section -->
  <div class="premium-test-pricing">
