...
class Controller extends BaseController
{
    ...
    public function getdefaultcityid($cityName = 'Jamnagar'){
        // Log::info($cityName);
        if($cityName == ''){
            $cityName = 'Jamnagar';
        }
        
        $defaultcityId = City::with('users')->where('default','Yes')->where('city',$cityName)->first();
        if (!$defaultcityId || $defaultcityId->users->isEmpty()) {
            return 0;
        }
        return $defaultcityId->users['0']->id;
    }
   ...
   public function send_notification_order_IOS($key,$user_id,$msg,$order_id){
    ...
    public function getsitedate(){
     ...
     public function ApplyCoupononUser($userId,$id,$subtotalcp,$cart_data){
    ...
    public function applycoupononsample_v2($cart_data,$id,$subtotalcp){
        log::info('total :-'.$subtotalcp);
        $price = $subtotalcp;
        $discount = 0;
        $coupon_data = Coupon::find($id);
        $today = date('l');
